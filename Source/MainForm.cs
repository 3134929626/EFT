...
namespace eft_dma_radar
{
    public partial class frmMain : MaterialForm
    {
        ...
        private void InitializeUIScaling()
        {
            ...
            this.aimviewWindowSize = 400 * this.uiScale;
        ...
        private void LoadConfig()
        {
            ...
            索敌距离.Value = this.config.AimDis;
            当前延迟.Value = this.config.GamePing;
        ...
        {
            if (this.config.Aimview || this.AllPlayers is not null)
            {
                ...
                if (aimviewPlayers is not null)
                {
                    ...
                    var localPlayerAimviewBoundsY = this.mcRadarLootItemViewer.Location.Y - this.aimviewWindowSize - 5;

                    var localPlayerAimviewBounds = new SKRect()
                    {
                        Left = (isItemListVisible ? localPlayerAimviewBoundsX : this.mapCanvas.Left),
                        Right = (isItemListVisible ? localPlayerAimviewBoundsX : this.mapCanvas.Left) + this.aimviewWindowSize,
                        Bottom = (isItemListVisible ? localPlayerAimviewBoundsY + this.aimviewWindowSize : this.mapCanvas.Bottom),
                        Top = (isItemListVisible ? localPlayerAimviewBoundsY : this.mapCanvas.Bottom - this.aimviewWindowSize)
                    };

                    var isRadarStatsVisible = this.mcRadarStats.Visible || this.mcRadarEnemyStats.Visible;
                    var primaryTeammateAimviewBoundsX = this.mcRadarStats.Location.X + this.mcRadarStats.Width - this.aimviewWindowSize;
                    var primaryTeammateAimviewBoundsY = this.mcRadarStats.Location.Y - this.aimviewWindowSize - 5;

                    var primaryTeammateAimviewBounds = new SKRect()
                    {
                        Left = (isRadarStatsVisible ? primaryTeammateAimviewBoundsX : this.mapCanvas.Right - this.aimviewWindowSize),
                        Right = (isRadarStatsVisible ? primaryTeammateAimviewBoundsX + this.aimviewWindowSize : this.mapCanvas.Right),
                        Bottom = (isRadarStatsVisible ? primaryTeammateAimviewBoundsY + this.aimviewWindowSize : this.mapCanvas.Bottom),
                        Top = (isRadarStatsVisible ? primaryTeammateAimviewBoundsY : this.mapCanvas.Bottom - this.aimviewWindowSize)
                    };
        ...
        private void RenderAimview(SKCanvas canvas, SKRect drawingLocation, Player sourcePlayer, IEnumerable<Player> aimviewPlayers)
        {
            ...
            if (loot is not null && loot.Filter is not null)
            {
                ...
                {
                    if (item is null || (this.config.ImportantLootOnly && !item.Important && !item.AlwaysShow && !item.RequiredByQuest))
                        continue;

                    if (this.ShouldDrawLootObject(myPosition, item.Position, this.config.MaxDistance))
                        this.DrawLootableObject(canvas, drawingLocation, myPosition, sourcePlayer.ZoomedPosition, item, normalizedDirection, pitch);
                }
            }
            ...
            if (Questloot is not null)
            {
                foreach (var item in Questloot)
                {
                    ...
                    if (this.ShouldDrawLootObject(myPosition, item.Position, this.config.MaxDistance))
                        this.DrawQuestableObject(canvas, drawingLocation, myPosition, sourcePlayer.ZoomedPosition, item, normalizedDirection, pitch);
                }
            }
        }
        ...
        private void DrawQuestableObject(SKCanvas canvas, SKRect drawingLocation, Vector3 myPosition, Vector2 myZoomedPos, QuestItem lootableObject, float normalizedDirection, float pitch)
        {
            ...
            if (IsInFOV(drawX, drawY, drawingLocation))
            {
                ...
                canvas.DrawCircle(drawX, drawY, circleSize * this.uiScale, paint);//SKPaints.LootPaint

                canvas.DrawText(lootableObject.Name, drawX, drawY - circleSize * this.uiScale / 2 - 2, paint);

                canvas.DrawText("[" + Math.Round(dist, 1) + "m]", drawX, drawY + circleSize * this.uiScale / 2 + paint.TextSize + 2, paint);
            }
        }
        ...
        public static bool F5 { get; set; } = false;
        public static bool F4 { get; set; } = false;
        private void 按键监控()
        {
            while (!cts.IsCancellationRequested)
            {
                ...
                if (F4)
                {
                    this.Invoke((MethodInvoker)delegate
                    {
                        var 状态 = swTimeScale.Checked;
                        swTimeScale.Checked = !状态;
                    });
                    Thread.Sleep(300);
                }
                Thread.Sleep(80);
        ...
        private void AimTeam_CheckedChanged(object sender, EventArgs e)
        {
            瞄准队友 = AimTeam.Checked;
        }
        public static bool 锁腿1 { get; set; }
        private void 锁腿_CheckedChanged(object sender, EventArgs e)
        {
            锁腿1 = 锁腿.Checked;
        }
        private void 索敌距离_onValueChanged(object sender, int newValue)
        {
            this.config.AimDis = newValue;
        }
