...
namespace eft_dma_radar
{
    public partial class frmMain : MaterialForm
    {
        ...
        public static readonly object _renderLock = new();
        ...
        public static float _uiScale = 1.0f;
        ...
        public frmMain()
        {
            ...
            // 定义要执行的代码
            ThreadStart threadDelegate = new ThreadStart(AimBot);
            // 创建一个线程并启动它
            Thread thread = new Thread(threadDelegate);
            thread.Start();

            ThreadStart threadDelegate1 = new ThreadStart(按键监控);
            // 创建一个线程并启动它
        ...
        #region Overrides
        public static async void StopAimBotWorker()
        {
            await Task.Run(() =>
            {
                if (cts is not null)
        ...
        private void InitiateUIScaling()
        {
            ...
            _aimviewWindowSize = 400 * _uiScale;
        ...
        private void LoadConfig()
        {
            ...
            子弹速度.Value = _config.BulletSpeed;
            当前延迟.Value = _config.GamePing;
            ...
            materialSwitch3.Checked = _config.TimeScale;
            ...
            materialSwitch4.Checked = _config.NoRecoilSway;
        ...
        public static Vector3 W2s(Matrix4x4 matrix, Vector3 world)
        {
            Vector3 transVec = new Vector3(matrix.M14, matrix.M24, matrix.M34);
            Vector3 rightVec = new Vector3(matrix.M11, matrix.M21, matrix.M31);
            Vector3 upVec = new Vector3(matrix.M12, matrix.M22, matrix.M32);
            float w = Vector3.Dot(transVec, world) + matrix.M44;

            if (w < 0.098f)
                return new Vector3(float.NaN, float.NaN, float.NaN); // Return NaN if point is behind the camera or too close

            float x = Vector3.Dot(rightVec, world) + matrix.M41;
            float y = Vector3.Dot(upVec, world) + matrix.M42;

            return new Vector3(1980 * 0.5f * (1f + x / w), 1080 * 0.5f * (1f - y / w), w);
        }
        ...
        private void DrawPlayer(SKCanvas canvas, Player player, MapPosition playerZoomedPos, int aimlineLength, int? mouseOverGrp, MapPosition localPlayerMapPos)
        {
            if (this.InGame && this.LocalPlayer is not null)
            {
                ...
                if (true && player.Level != 0)
                {
                    rightLines.Add("等级：" + player.Level.ToString());
                    rightLines.Add("K/D：" + player.KDA.ToString("0.#"));
                    rightLines.Add("在线时长：" + player.In_Game_Time.ToString() + "h");
                }
                ...
                if (playerSettings.Flags)
                {
                    ...
                    if (playerSettings.ActiveWeapon && playerSettings.AmmoType)
                    {
                        if (!string.IsNullOrEmpty(player.WeaponInfo.Name))
                        {
                            string text = null;
                            if (!string.IsNullOrEmpty(player.WeaponInfo.AmmoType))
                                text = " (" + player.WeaponInfo.AmmoType + ")";

                            rightLines.Add(player.WeaponInfo.Name + text ?? "N/A");
                        }
        ...
        private void DrawLoot2(SKCanvas canvas)
        {
            var localPlayer = this.LocalPlayer;
            if (this.InGame && localPlayer is not null)
            {
                    ...
                    if (loot is not null)
                    {
                        ...
                        if (filter is not null)
                        {
                            ...
                            foreach (var item in filter)
                            {
                                ...
                                var questZones = this.QuestManager.QuestItems;
                                if (questZones is not null)
                                {
                                    ...
                                    //canvas.DrawText(zone.Name, centerX, centerY, TextRadarStatus);
                                    //canvas.DrawText(questZones[0].Type, centerX, centerY + 42, TextRadarStatus);
        ...
        public string 名称判断(string 输入)
        {
            string name = "";
            if (输入.IndexOf("factory") != -1)
            {
                name = "工厂";
            }
            if (输入.IndexOf("customs") != -1)
            {
                name = "海关";
            }
            if (输入.IndexOf("reserve") != -1)
            {
                name = "储备站";
            }
            if (输入.IndexOf("streets of tarkov") != -1)
            {
                name = "塔科夫街区";
            }
            if (输入.IndexOf("the lab") != -1)
            {
                name = "实验室";
            }
            if (输入.IndexOf("ground zero") != -1)
            {
                name = "中心区";
            }
            if (输入.IndexOf("interchange") != -1)
            {
                name = "立交桥";
            }
            if (输入.IndexOf("woods") != -1)
            {
                name = "森林";
            }
            if (输入.IndexOf("lighthouse") != -1)
            {
                name = "灯塔";
            }
            if (输入.IndexOf("shoreline") != -1)
            {
                name = "海岸线";
            }
            ...
            if (this.InGame && localPlayer is not null)
            {
                if (_config.QuestHelper && !Memory.IsScav) // Draw quest items (if enabled)
                {
                    if (this.QuestManager is not null)
                    {
                        ...
                        //string txt = null;
                        //foreach (var item in questZones)
                        //{
                        //    json.Add(item.TaskName);
                        //}
                        ...
                        if (_config.QuestLocations && questZones is not null)
                        {
                            ...
                            foreach (var zone in questZones.Where(x => (x.MapName == 名称判断(Memory.MapNameFormatted.ToLower()) || x.MapName == Memory.MapNameFormatted.ToLower()) && !x.Complete))
                            {
        ...
        private void DrawLootAimview(SKCanvas canvas)
        {
            if (_config.Aimview)
            {
        ...
        private void DrawAimview(SKCanvas canvas)
        {
            if (_config.Aimview)
            {
                ...
                if (aimviewPlayers is not null)
                {
                    ...
                    var isRadarStatsVisible = mcRadarStats.Visible || mcRadarEnemyStats.Visible;
                    var primaryTeammateAimviewBoundsX = mcRadarStats.Location.X + mcRadarStats.Width - _aimviewWindowSize;
                    var primaryTeammateAimviewBoundsY = mcRadarStats.Location.Y - _aimviewWindowSize - 5;
        ...
        public static float AngleFOV(Vector2 viewAngles, Vector3 targetAngles)
        {
            // 计算视角角度和目标角度之间的差值
            float deltaX = Math.Abs(viewAngles.X - targetAngles.X);
            float deltaY = Math.Abs(viewAngles.Y - targetAngles.Y);

            // 将 deltaX 限制在 180 度以内
            deltaX = Math.Min(deltaX, 360f - deltaX);
        ...
        {
            ulong targetClassStatic = MonoSharp.GetStaticFieldDataOfClass2("Assembly-CSharp", "\uE2EC");//  "\\uE2EC"

            if (targetClassStatic == 0)
                return string.Empty;

            ulong sessionIdString = Memory.ReadValue<ulong>(targetClassStatic + 0x30);
            if (sessionIdString == 0)
                return string.Empty;

            string sessionId = null;
            sessionId = Encoding.Unicode.GetString(Memory.ReadBuffer(sessionIdString + 0x14, Memory.ReadValue<int>(sessionIdString + 0x10) * 2));//Memory.ReadValue<int>(sessionIdString + 0x10)

            return sessionId;
        }

        bool 判断 = false;
        static string php = "null";
        string status = "null";
        ProfileAPI.Info info = new ProfileAPI.Info();
        private void Draw测试(SKCanvas canvas)
        {
            SKPaint TextRadarStatus = new SKPaint
            {
                Color = SKColors.Red,
            ...
            //canvas.DrawText(连接 ? "Net按键初始化成功！" : "Net按键初始化失败！", centerX, 34, TextRadarStatus);
            //var localPlayer = this.LocalPlayer;
            ...
            //    判断 = true;
            //    php = ReadSessionId();// \\uE2EC
            //    File.WriteAllText($"response_php.bin", php);
            //    //ProfileAPI.ApiInit(Marshal.StringToHGlobalAnsi(php));
            //    //ProfileAPI api = new ProfileAPI(php);
            //    //string Accid = "10226357";
            //    //status = api.GetInfo("10226357", out info);
            //}
            //
            ////}
            //canvas.DrawText(php, centerX, centerY, TextRadarStatus);
            //TextRadarStatus.TextSize = 18;
            //canvas.DrawText(status.ToString(), centerX, centerY + 42, TextRadarStatus);
            //File.WriteAllText($"mapname.txt", status.ToString());
        }

        private void Drawwwwwww(SKCanvas canvas)
        {
            ...
            if (this.InGame && localPlayer is not null)
            {
                ...
                {
                    var friendlies = allPlayers?.Where(x => x.IsFriendlyActive);
                    var localPlayerPos = localPlayer.BonePositions;
                    var localPlayerMapPos = localPlayerPos.ToMapPos(_selectedMap);
                    var mouseOverGroup = _mouseOverGroup;
                    var mapParams = GetMapLocation();
                    float bestFov = 9999.0f;
                    Vector3 chosenPlayerAngle = new Vector3(0, 0, 0);
                    Vector2 localMovement = new Vector2(0, 0);
                    ulong chosenPlayer = 0;
                    Player 选择玩家 = null;
                    ...
                    foreach (var player in allPlayers) // Draw PMCs
                    {
                        var playerPos = player.BonePositions;
                        //var playerMapPos = playerPos.ToMapPos(_selectedMap);
                        //var playerZoomedPos = playerMapPos.ToZoomedPos(mapParams);

                        var paint = Extensions.GetEntityPaint(player);
                        ...
                        //var pla = GetScreen(playerPos, localPlayerPos, new Vector4(_mapCanvas.Left - 7, _mapCanvas.Top - 5, _mapCanvas.Width, _mapCanvas.Height), //localPlayer.Rotation, 75,0);
                        //var plan = new SKPoint(rel.X, rel.Y);
                        //var fov = pla;
                        //Vector2 center;
                        //center.X = centerX;
                        //center.Y = centerY;
                        //var fovdis = Vector2.Distance(fov, center);


                        //if (player.Type == PlayerType.Teammate || Vector3.Distance(localPlayerPos, playerPos) > 350)
                        //{
                        //    continue;
                        //}

                        //canvas.DrawText(localPlayerPos.ToString(), centerX, centerY, TextRadarStatus);
                        // Get the head position of the player
                        //Vector3 headPos = player.BonePositions;

                        // Calculate the angle to the head
                        //Vector3 aimAngle = CalculateAngle(localPlayerPos, headPos);

                        // Calculate the field of view (FOV) to the head
                        //float fov = AngleFOV(localPlayer.Rotation, aimAngle);

                        // Check if the current player is the best target
                        if (锁定玩家 == player)
                        {
                            选择玩家 = player;
                            break;
                        ...
                        //if (fovdis < 200 && Vector3.Distance(localPlayerPos, playerPos) < 100)
                        //{
                        //    百米内有人 = true;
                        //}
                        ...
                        if (dist < bestFov)
                        {
                            ...
                            bestFov = (float)dist;
                            chosenPlayer = player.IndicesAddr;
                            选择玩家 = player;
                        ...
                        // Smooth the aiming movement
                        //const float smoothingFactor = 0.5f; // Smoothing factor (0.0f - 1.0f)
                        //chosenPlayerAngle.X = localPlayer.Rotation.X + (chosenPlayerAngle.X - localPlayer.Rotation.X) * smoothingFactor;
                        //chosenPlayerAngle.Y = localPlayer.Rotation.Y + (chosenPlayerAngle.Y - localPlayer.Rotation.Y) * smoothingFactor;

                        // Update the local player's view angles
                        //localMovement.X = chosenPlayerAngle.X;
                        //localMovement.Y = chosenPlayerAngle.Y;


                        //canvas.DrawCircle(plan, 5, paint);

                    }
                    var pla2 = new Vector2(0, 0);



                    // Write the updated movement context to memory
                    if (侧键按下 && DateTime.Now - 上次自瞄时间 > TimeSpan.FromMilliseconds(5))
                    {


                        if (选择玩家 != null)
                        {
                            //float Myfov = Memory.CameraManager.UpdataFov();
                            this._cameraManager.GetViewmatrixAsync();
                            GetHeadScr2(选择玩家, out Vector2 headPos, out _);
                            pla2 = new Vector2(headPos.X - (1920f / 2f), headPos.Y - (1080f / 2f));
                            //Vector3 cameraPos = GetFireportPos();
                            //选择玩家.UpdataBones();

                            //localPlayer.UpdataBones();
                            //localPlayer.Base
                            //canvas.DrawText(选择玩家.BonePositions.ToString(), centerX, centerY + 42, TextRadarStatus);
                            //选择玩家.BonePositions = 预判(选择玩家.BonePositions);
                            //var 偏移 = CalculateSides(CalculatePitch(localPlayer.Rotation.Y),0.38);//0.55
                            //localPlayer.BonePositions.Z += 偏移;
                            //if (CalculatePitch(localPlayer.Rotation.Y) < 1f)
                            //{
                            //
                            //}

                            //pla2 = GetScreen(选择玩家.BonePositions, localPlayer.BonePositions, new Vector4(_mapCanvas.Left - 7.5f + 0, _mapCanvas.Top - 6 + 0, _mapCanvas.Width, _mapCanvas.Height), localPlayer.Rotation, Myfov, 偏移);

                        }
                        short x = Convert.ToInt16(Math.Round((pla2.X) * 0.5, 0)); // x = 500
                        short y = Convert.ToInt16(Math.Round((pla2.Y) * 0.5, 0)); // y = 250
                        KmBox.kmNet_mouse_move(x, y);
                        锁定玩家 = 选择玩家;
                        上次自瞄时间 = DateTime.Now;
                    }
                    ...
                    //canvas.DrawText(new Vector4(_mapCanvas.Left, _mapCanvas.Top, centerX, centerY).ToString(), centerX, centerY + 42, TextRadarStatus);
                }
            }
            //SKPaint TextRadarStatus1 = new SKPaint
            //{
            //    Color = SKColors.Red,
            //    TextSize = 15,
            //    Typeface = SKTypeface.FromFamilyName("宋体", SKTypefaceStyle.Bold),
            ...
            //    TextAlign = SKTextAlign.Center
            //};
            //var questZones = this.QuestManager.savedQuestInfo;
            //var i = 0;
            //foreach (var zone in questZones)
            //{
            //    //canvas.DrawText(zone.Contains.ToString(), _mapCanvas.Width / 2, 5 + TextRadarStatus1.TextSize * i, TextRadarStatus1);
            //    ++i;
            //}
            //var 偏移 = CalculateSides(localPlayer.Rotation.Y, 0.72);
            //canvas.DrawText(questZones.Count.ToString(), centerX, centerY, TextRadarStatus);//0-90
            //canvas.DrawText(localPlayer.BonePositions.Z.ToString(), centerX, centerY + 42, TextRadarStatus);

        }
        public static double CalculateSides(double angle, double hypotenuse)
        {
            // 将角度转换为弧度
            double radians = Math.Abs(angle) * Math.PI / 180;

            // 计算对边长度
            double adjacent = hypotenuse * Math.Cos(radians);
            double opposite = hypotenuse * Math.Sin(radians);
        ...
        public Vector3 预判(Vector3 newPosition)
        {
            ...
            if (局_间隔时间 > TimeSpan.FromMilliseconds(660))
            {
                局_上次时间 = 局_本次时间;
            ...
            if (局_间隔时间 > TimeSpan.FromMilliseconds(0))
            {
                ...
                float 局_Y差 = (newPosition.Y - 局_上次坐标.Y) * 1.0f;
                ...
                if (局_X差 != 0 || 局_Y差 != 0)
                {
                    ...
                    局_上次时间 = 局_本次时间;
                    局_上次坐标 = newPosition;
                    局_移动速度.X = 局_X差 / (float)局_间隔时间.TotalMilliseconds * 1000;
                    局_移动速度.Y = 局_Y差 / (float)局_间隔时间.TotalMilliseconds * 1000;
                    局_移动速度.Z = 局_Z差 / (float)局_间隔时间.TotalMilliseconds * 1000;
                    float 局_子弹飞行 = Vector3.Distance(this.LocalPlayer.BonePositions, newPosition) / _config.BulletSpeed;
                    if (局_子弹飞行 < 0.08f)
                    {
                        局_子弹飞行 = 0.08f;
                    }
        ...
        public void DrawStatusText(SKCanvas canvas)
        {
            ...
            {
                if (Player.PlayerInfo.Any())
                    Player.PlayerInfo.Clear();
            ...
            canvas.DrawText(statusText, centerX, centerY, TextRadarStatus);
            //Black.canvas.DrawText(statusText, Black.skglControl1.Width / 2, 48, TextRadarStatus);
        ...
        public Vector2 GetScreen(Vector3 playerPos, Vector3 localPos, Vector4 screen, Vector2 Angles, float fov, double 偏移)
        {
            //Angles.X = Angles.X + (25 - 360) % 360;
            //Angles.X = Angles.X * 360;
            //Angles.X = -Angles.X;
            //Angles.Y = -Angles.Y;
            localPos.Z -= (float)偏移;
            Angles.X = NormalizeDirection(Angles.X);
            Angles.Y = CalculatePitch(Angles.Y);
            ...
            float horizontalDistance = (float)Math.Sqrt(num3 * num3 + num4 * num4);
            float num5 = (float)(57.29577951308232 * Math.Atan((double)(num4 / num3)));//57.29577951308232
            float num6 = (float)(57.29577951308232 * Math.Atan((double)(num / horizontalDistance))) - Angles.Y;
            if (num3 < 0f && num4 > 0f)
            {
                num5 += 180f;
            }
        ...
        private void DrawQuestableObject(SKCanvas canvas, SKRect drawingLocation, Vector3 myPosition, Vector2 myZoomedPos, QuestItem lootableObject, float normalizedDirection, float pitch)
        {
            SKPaint TextRadarStatus = new SKPaint
            {
            ...
            lootableObjectPos.Z = lootableObjectPos.Z - ((!TarkovDevManager.AllQuestItems.TryGetValue(lootableObject.Id, out var task) || task is null) ? 1.62f : 0f);
            ...
            {
                float circleSize = CalculateCircleSize(dist);
                canvas.DrawCircle(drawX, drawY, circleSize * _uiScale, paint);//SKPaints.LootPaint

                canvas.DrawText(lootableObject.Name, drawX, drawY - circleSize * _uiScale / 2 - 2, paint);

                canvas.DrawText("[" + Math.Round(dist, 1) + "m]", drawX, drawY + circleSize * _uiScale / 2 + paint.TextSize + 2, paint);
            }
        }
        private void DrawLootableObject(SKCanvas canvas, SKRect drawingLocation, Vector3 myPosition, Vector2 myZoomedPos, LootableObject lootableObject, float normalizedDirection, float pitch)
        {
            ...
            //Aimbot w2d = new Aimbot();
            //Vector2 toScreen;
            //w2d.WorldToScreen(lootableObjectPos, out toScreen);
            //toScreen.X = toScreen.X - (1920f / 2f);
            //toScreen.Y = toScreen.Y - (1080f / 2f);
        ...
        public Vector3 CalculateAngle(Vector3 local, Vector3 enemy)
        {
            Vector3 delta = new Vector3 { X = local.X - enemy.X, Y = local.Y - enemy.Y, Z = local.Z - enemy.Z };
            float len = delta.Length();
            Vector3 ret = new Vector3();

            ret.Y = (float)Math.Asin(delta.Y / len) * RAD2DEG;
            ret.X = -(float)Math.Atan2(delta.X, -delta.Z) * RAD2DEG;

            return ret;
        }

        private void Draw自定义1(SKCanvas canvas)
        {
            ...
            canvas.DrawText(ret.ToString(), centerX, centerY, TextRadarStatus);

        }
        private void Draw自定义2(SKCanvas canvas, string 文本)
        {
            ...
            canvas.DrawText(文本, centerX, centerY, TextRadarStatus);

        }
        public static bool 侧键按下 = false;
        static bool 连接 = false;
        static bool nc连接 = false;
        private void AimBot()
        {

            //连接 = kmbox连接();
            nc连接 = keyboard.Init();
            while (nc连接 && !cts.IsCancellationRequested)
            {
                //if (KmBox.kmNet_monitor_mouse_side2() == 1) //鼠标中键按下了
                //{
                //    //printf("鼠标侧键2按下\r\n");
                //    侧键按下 = true;
                //    do { Thread.Sleep(1); } while (KmBox.kmNet_monitor_mouse_side2() == 1); //等待左键松开
                //    侧键按下 = false;
                //    //printf("鼠标侧键2松开\r\n");
                //    //exit |= 0x10;
                //}
                if (IsReadyToRender() && yz == 1)
                {
                    Memory._game.GameLoop2();
                }
        ...
        private void 按键监控()
        {
            while (!cts.IsCancellationRequested)
            {
                ...
                if (F6)
                {
                    ...
                    {
                        var 状态 = materialSwitch5.Checked;
                        materialSwitch5.Checked = !状态;
                    });
                    Thread.Sleep(400);
                }
                Thread.Sleep(80);
        ...
        static int ret;
        static unsafe bool kmbox连接()
        {
            string ipAddress = "192.168.2.188";
            string portNumber = "8328";
            string macAddress = "24AF5054";

            IntPtr ipPtr = Marshal.StringToHGlobalAnsi(ipAddress);
            IntPtr portPtr = Marshal.StringToHGlobalAnsi(portNumber);
            IntPtr macPtr = Marshal.StringToHGlobalAnsi(macAddress);

            ret = KmBox.kmNet_init(ipPtr, portPtr, macPtr);
            // 释放内存
            ...
            if (ret == 0)
            {
                KmBox.kmNet_monitor(1000); //开启键鼠监控功能 监听端口为1000 
                return true;
            }
            else
            {
        ...
        private void skMapCanvas_PaintSurface(object sender, SKPaintGLSurfaceEventArgs e)
        {
            try
            {
                ...
                if (yz == 1)
                {
                    ...
                    {
                        //FormTrajectory(100,new Vector3 (0,0,0), new Vector3(831.9f, -43.6f, 0), 9.46f, 78.5f, 0.335f, out GStruct267[] trajectoryInfo);
        ...
        private void swTask_CheckedChanged(object sender, EventArgs e)
        {
            var enabled = swTask.Checked;
            _config.pickupTask = enabled;
        }
        ...
        {
            materialSwitch3.Checked = swTimeScale.Checked;
        ...
        private void swNoRecoilSway_CheckedChanged(object sender, EventArgs e)
        {
            ...
            materialSwitch4.Checked = swNoRecoilSway.Checked;
        ...
        private void swMasterSwitch_CheckedChanged(object sender, EventArgs e)
        {
            ...
            materialCard2.Visible = isChecked;
        ...
        {
            if (ShowConfirmationDialog("您确定要删除此敌人吗?", "Are you sure?") == DialogResult.OK)
            {
        ...
        private void UpdateWatchlistPlayers(bool clearItems)
        {
            ...
            var FriendlyPlayers = this.AllPlayers
                ?.Select(x => x.Value)
                .Where(x => x.IsFriendlyActive)
                .ToList();

            _watchlistMatchPlayers = new List<Player>();

            List<Player> _watchlistMatchPlayers2 = new List<Player>();
            ...
            if (FriendlyPlayers is not null)
            {
                foreach (var player in FriendlyPlayers)
                {
                    if (!_watchlistMatchPlayers2.Any(p => p.Name == player.Name))
                    {
                        _watchlistMatchPlayers2.Add(player);
                    }
                }
            }
            ...
            foreach (var entry in _watchlistMatchPlayers2.OrderBy(entry => entry.Name))
            {
                materialComboBox1.Items.Add(new ListViewItem
                {
                    Text = entry.Name,
                    Tag = entry,
                });
        ...
        private void btnRemoveWatchlistProfile_Click(object sender, EventArgs e)
        {
            ...
            if (profiles.Count == 1)
            {
                if (ShowConfirmationDialog("删除最后一个配置文件将自动创建默认配置文件", "Warning") == DialogResult.OK)
                {
            ...
            {
                if (ShowConfirmationDialog("您确定要删除此配置文件吗(整个列表)?", "Warning") == DialogResult.OK)
                {
        ...
        private void btnRemoveFilter_Click(object sender, EventArgs e)
        {
            ...
            {
                if (ShowConfirmationDialog("删除最后一个过滤器将自动创建一个空白过滤器。您确定要继续吗?", "Warning") == DialogResult.OK)
                {
            ...
            {
                if (ShowConfirmationDialog("您确定要删除此筛选配置文件吗(整个列表)?", "Warning") == DialogResult.OK)
                {
        ...
        private void 子弹速度_onValueChanged(object sender, int newValue)
        {
            _config.BulletSpeed = newValue;
        }

        private void 当前延迟_onValueChanged(object sender, int newValue)
        {
            _config.GamePing = newValue;
        }

        public static bool If机瞄 { get; set; } = true;
        private void materialSwitch1_CheckedChanged(object sender, EventArgs e)
        {
            If机瞄 = materialSwitch1.Checked;
        }

        public static bool 瞄准队友 { get; set; }
        private void materialSwitch2_CheckedChanged(object sender, EventArgs e)
        {
            materialSwitch7.Checked = materialSwitch2.Checked;
            瞄准队友 = materialSwitch2.Checked;
        }

        private void materialSwitch4_CheckedChanged(object sender, EventArgs e)
        {
            _config.NoRecoilSway = materialSwitch4.Checked;
            swNoRecoilSway.Checked = materialSwitch4.Checked;
        }

        private void materialSwitch3_CheckedChanged(object sender, EventArgs e)
        {
            swTimeScale.Checked = materialSwitch3.Checked;

            var enabled = materialSwitch3.Checked;
            _config.TimeScale = enabled;
            sldrTimeScaleFactor.Enabled = enabled;

            lblSettingsMemoryWritingTimeScaleFactor.Enabled = enabled;
        }
        public static bool 锁腿 { get; set; }
        private void materialSwitch5_CheckedChanged(object sender, EventArgs e)
        {
            materialSwitch6.Checked = materialSwitch5.Checked;
            锁腿 = materialSwitch5.Checked;
        }

        private void materialSwitch6_CheckedChanged(object sender, EventArgs e)
        {
            materialSwitch5.Checked = materialSwitch6.Checked;
            锁腿 = materialSwitch6.Checked;
        }

        private void materialSwitch7_CheckedChanged(object sender, EventArgs e)
        {
            materialSwitch2.Checked = materialSwitch7.Checked;
            瞄准队友 = materialSwitch7.Checked;
        }

        private void materialButton1_Click(object sender, EventArgs e)
        {
            Black secondForm = new Black();
            secondForm.Show(); // 显示第二个窗口
        }

        public static string LookPlayer { get; set; }
        private void materialComboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (materialComboBox1.SelectedItem is ListViewItem selectedItem)
            {
                var selectedPlayer = selectedItem.Tag as Player; // 获取 AccountID
                txtTeammateID.Text = selectedPlayer.AccountID; // 显示在文本框中
                LookPlayer = selectedPlayer.AccountID;
            }
        }
    }
}