...
namespace eft_dma_radar
{
    ...
    public struct MapPosition
    {
        ...
        public void DrawTaskZone(SKCanvas canvas, QuestZone zone, float heightDiff)
        {
            ...
            tooltipText.Insert(0, zone.TaskName);
        ...
        public void DrawToolTip(SKCanvas canvas, QuestItem item)
        {
            ...
            foreach (var line in lines)
            {
                canvas.DrawText(line, left + padding, y, TextBase);
                y -= textSpacing;
            }
        }
        ...
        private void DrawToolTip(SKCanvas canvas, LootItem lootItem)
        {
            ...
            var left = X + padding;
            var top = Y - padding;
            var right = left + width * 1.2f + padding * 2;
            var bottom = top + height + padding * 2;
        ...
        //    var left = X + padding;
        //    var top = Y - padding;
        //    var right = left + maxWidth * 1.2f + padding * 2;
        //    var bottom = top + height + padding * 2;
        ...
        private void DrawToolTip(SKCanvas canvas, LootCorpse corpse)
        {
            ...
            //if (!isEmptyCorpseName)
            //{
                canvas.DrawText(corpse.Name, left + padding, y, TextBase);
                y -= textSpacing;
            //}
        }
        ...
        private void DrawHostileTooltip(SKCanvas canvas, Player player)
        {
            ...
            if (player.Gear is not null)
            {
                foreach (var slot in GearManager.GEAR_SLOT_NAMES)
                {
                    if (player.Gear.TryGetValue(slot.Key, out var gearItem))
                    {
                        ...
                        {
                            lines.Insert(0, $"{slot.Value}: {itemName.Trim()}-任务");
                            continue;
                        }
            ...
            lines.Insert(0, $"价值: {TarkovDevManager.FormatNumber(player.Value)}");
        ...
        private void DrawToolTip(SKCanvas canvas, string tooltipText)
        {
            ...
            {
                var line1 = line;
                if (line1.Contains("-任务"))
                { 
                    line1 = line.Substring(0, line.Length - 3);
                    TextBase.Color = SKColors.HotPink;
                }else
                    TextBase.Color = SKColors.WhiteSmoke;

                canvas.DrawText(line1, left + padding, y, TextBase);
                y -= textSpacing;
            }
        }
    }
